...
if(isset($_SESSION['memberID'])&&$_SESSION['accountType']==="member"){
	...
	$select = "SELECT * FROM member WHERE member_id = '$memberID' AND member_name='$memberName'";
	...
	if(isset($_POST['btnUpdate'])){
		...
		$member_password = $_POST['txtMemberPassword'];
		$member_address = $_POST['txtMemberAddress'];
		$member_phonenumber = $_POST['txtMemberPhoneNumber'];


		echo $update="UPDATE member SET member_name='$member_name',
		member_email='$member_email',member_password='$member_password',member_address='$member_address',member_phonenumber='$member_phonenumber'
	...
	<form action="memberUpdate.php" method = "POST" enctype="multipart/form-data">
		...
		<table>
			...
			<tr>
				<td>Set Password</td>
				<td>
					<input type="password" name="txtMemberPassword" required value="<?php echo $member_password ?>">
				</td>
			</tr>
			<tr>
				<td>Phone Number</td>
